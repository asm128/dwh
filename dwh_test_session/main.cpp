...
int												main									()					{
	...
	// ----------- Client reports to authority and receives keys and session information - BEGIN
		gpk_necall(result = ::dwh::authorityClientIdentifyRequest			(client		, output)		, "%s", "Unknown error.");	info_printf("Client    -> authorityClientIdentifyRequest        result: %x.", result);	// 0. Client sends a request to the authority server and software identifier.																
		gpk_necall(result = ::dwh::authorityServerIdentifyResponse			(authority	, output, input), "%s", "Unknown error.");	info_printf("Authority -> authorityServerIdentifyResponse       result: %x.", result);	// 1. Authority's server sends a request with the public key for session startup.															
	// ----------- Client reports to authority and receives keys and session information - END

	// ----------- Client connects to the service - BEGIN
		gpk_necall(result = ::dwh::sessionClientStart						(client		, input, output), "%s", "Unknown error.");	info_printf("Client    -> sessionClientStart                    result: %x.", result);	// 2. Client sends a request to the service's server.																						
	// ----------- Server checks with authority for validity of the client requesting connection - BEGIN
		gpk_necall(result = ::dwh::authorityServiceConfirmClientRequest		(service	, output, input), "%s", "Unknown error.");	info_printf("Server    -> authorityServiceConfirmClientRequest  result: %x.", result);	// 3. Service server sends a request to authority server with client information.															
		gpk_necall(result = ::dwh::authorityServerConfirmClientResponse		(authority	, input, output), "%s", "Unknown error.");	info_printf("Authority -> authorityServerConfirmClientResponse  result: %x.", result);	// 4. Authority server sends a response to service with client information and the private key for decrypting the client password.			
	// ----------- Server checks with authority for validity of the client requesting connection - END
		gpk_necall(result = ::dwh::sessionServerAccept						(service	, output, input), "%s", "Unknown error.");	info_printf("Server    -> sessionServerAccept                   result: %x.", result);	// 5. Service server sends a response to the client containing the symmetric keys for the rest of the communication.						
		gpk_necall(result = ::dwh::sessionClientAccepted					(client		, input, output), "%s", "Unknown error.");	info_printf("Client    -> sessionClientAccepted                 result: %x.", result);	// 6. Client processes service response in order to determine if the connection was accepted as legitimate and loads the symmetric keys.	
	// ----------- Client connects to the service - END
		info_printf("Client connected. Symmetric key: %llx.", client.KeySymmetric);
