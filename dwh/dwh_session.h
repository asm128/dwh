...
// ----------- Client connects to the service - END
	enum DWH_SESSION_STAGE 
		{ DWH_SESSION_STAGE_CLIENT_CLOSED					= 0	// authorityClientIdentifyRequest		()
		, DWH_SESSION_STAGE_CLIENT_IDENTIFY						// authorityClientIdentifyRequest		()
	...
	struct SSessionClient		{ 
		DWH_SESSION_STAGE						Stage										= DWH_SESSION_STAGE_CLIENT_CLOSED; 
		uint64_t								ClientId									= (uint64_t)-1LL;
	};
	...
	::gpk::error_t							authorityServiceConfirmClientRequest		(::dwh::SSessionService		& service	, ::gpk::view_array<byte_t> & input, ::gpk::array_pod<byte_t> & output);	// 3. Service server sends a request to authority server with client information.															
	...
	::gpk::error_t							sessionServerAccept							(::dwh::SSessionService		& service	, ::gpk::view_array<byte_t> & input, ::gpk::array_pod<byte_t> & output);	// 5. Service server sends a response to the client containing the symmetric keys for the rest of the communication.						
	...
	::gpk::error_t							sessionClientEncrypt						(::dwh::SSessionClient		& client	, ::gpk::view_array<byte_t> & input, ::gpk::array_pod<byte_t> & output);	// 7. Client processes service response in order to determine if the connection was accepted as legitimate and loads the symmetric keys.	
	::gpk::error_t							sessionClientDecrypt						(::dwh::SSessionClient		& client	, ::gpk::view_array<byte_t> & input, ::gpk::array_pod<byte_t> & output);	// 8. Client processes service response in order to determine if the connection was accepted as legitimate and loads the symmetric keys.	
}
